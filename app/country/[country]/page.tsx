...
export default async function CountryLandingPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-[#F3F0E9]">
      ...
      {/* Categories */}
      <CategoriesSection countryName={config.name} />
  ...
  const props = [
    {
      ...
      description: 'Our local representatives are on hand throughout your stay to ensure everything runs smoothly and to offer insider tips on the best local experiences.',
    },
    {
      icon: Search,
      title: 'Personally inspected',
      description: 'Every villa in our collection has been personally inspected by our team. We know each property intimately and can advise on the perfect choice for your needs.',
    },
    {
      icon: ShieldCheck,
      title: 'ABTA protected',
      description: 'Book with confidence knowing your holiday is fully protected. We are members of ABTA, providing you with financial security and peace of mind.',
    },
  ];
    ...
    <section className="bg-[#F3F0E9] py-20 px-6 md:px-20">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
        {CATEGORIES.map((cat) => (
          <Link
            key={cat.id}
            href={`${cat.href}&country=${encodeURIComponent(countryName)}`}
